...
public class posbootrec implements InventoryHolder {
    private Inventory inv;

    public posbootrec() {
        inv = Bukkit.createInventory(this, 54, "Poseidon's Boots Recipe");
        init();
    }

    private void init() {
        inv.setItem(24, utils.blacklistitem(itemmanager.posboots));

        inv.setItem(10, utils.blacklistitem(new ItemStack(Material.BEACON, 1))); //Slot 1
        inv.setItem(11, utils.blacklistitem(new ItemStack(Material.DIAMOND_BOOTS, 1))); //Slot 2
        inv.setItem(12, utils.blacklistitem(new ItemStack(Material.ENCHANTING_TABLE, 1))); //Slot 3
